...
class LousaDigital:
    ...
    def setup_buttons(self):
        ...
        self.botoes_cores = []
        x_start = 50
        for i, (nome, cor) in enumerate(list(self.cores.items())[:6]):  # Primeiras 6 cores
            x = x_start + i * 120
            self.botoes_cores.append([x, 20, x + 100, 60, cor, nome])
        
        # Botões de ferramentas (segunda linha)
        self.botoes_ferramentas = [
            [50, 80, 130, 120, (128, 128, 128), "Limpar"],
            [150, 80, 230, 120, (128, 128, 128), "Salvar"],
            [250, 80, 330, 120, (128, 128, 128), "Carregar"],
            [350, 80, 410, 120, (128, 128, 128), "+"],
            [430, 80, 490, 120, (128, 128, 128), "-"],
            [510, 80, 590, 120, (128, 128, 128), "Desfazer"]
        ]
    ...
    def carregar_desenho(self):
        ...
        arquivos = [f for f in os.listdir('.') if f.startswith('desenho_') and f.endswith('.json')]
        if not arquivos:
        ...
        try:
            ...
            for ponto in desenho_carregado:
                self.desenho.append((
                    ponto['x'], ponto['y'],
                    tuple(ponto['cor']), ponto['espessura']
                ))
    ...
    def desenhar_interface(self, img):
        ...
        for bx1, by1, bx2, by2, bcor, texto in self.botoes:
            ...
            if texto in self.cores and self.cores[texto] == self.cor:
                cv2.rectangle(img, (bx1-3, by1-3), (bx2+3, by2+3), (255, 255, 255), 3)
            ...
            cv2.putText(img, texto, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
        
        # Informações na tela
        info_y = 150
        cv2.putText(img, f"Espessura: {self.espessura}", (50, info_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        cv2.putText(img, f"Pontos: {len(self.desenho)}", (50, info_y + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        ...
        for i, instrucao in enumerate(instrucoes):
            cv2.putText(img, instrucao, (50, img.shape[0] - 80 + i*25), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
